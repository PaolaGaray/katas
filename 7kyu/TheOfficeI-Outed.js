...
const outed = (meet, boss) =>{
    ...
    for (let person in meet) {
        if (person === boss) sumHappiness += meet[person] * 2;
        else sumHappiness += meet[person];
    }
    if (sumHappiness / Object.values(meet).length <= 5) return 'Get Out Now!'
    return 'Nice Work Champ!'
}


//Test:
outed({'tim':0, 'jim':2, 'randy':0, 'sandy':7, 'andy':0, 'katie':5, 'laura':1, 'saajid':2, 'alex':3, 'john':2, 'mr':0}, 'saajid')
//'Get Out Now!'
